...
/**
 * @brief Calculates the coordinates of edges of a triangle, defined by the input constraints using the Newton-Raphson
 * method
 *
 * @note a modified version of the Newton-Raphson is used where we calculate the equation: J_f(x_k)s_k = -f(x_k) is
 * solved and the next iteration will use: x_k+1 = x_k + s_k
 *
